...
class TextMulClassify(object):
    ...
    @cached_property
    def classify(self): return self # alias
    ...
    def __init__(self, model1, tags_file1, **opts):
        ...
        for opt1, v1 in self.opts.iteritems(): setattr(self, opt1, v1)
    ...
    def cpath(self, name): return self.model.pickle_path(name)
...
###############################################################################################################
    def evaluate_effect(self, default_guess_count=10):
        ...
        def inspect_result(result, filter_item_ids=set([])):
            for idx1, two_parts in enumerate(result):
                ...
                if recommend_data['item_id'] not in filter_item_ids: continue
    ...
    def load_train_data(self):
        ...
        if not os.path.exists(cpath('idf')): self.model.pull_data()

        for idx1, data1 in enumerate(data_list):
            print "\n"*5; print "_"*150; print "["*3 + "#"*144 + "]"*3
            print "[第%i步] 检查预生成 %s" % (idx1+1, data1['path'])

            if not isinstance(data1['path'], list): data1['path'] = [data1['path']]

            should_read_attr = False
            for path1 in data1['path']:
                if not os.path.exists(path1): should_read_attr = True
